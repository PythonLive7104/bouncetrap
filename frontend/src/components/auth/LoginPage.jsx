...
export default function LoginPage() {
  ...
  async function handleSubmit(ev) {
    ...
    try {
      ...
      if (!err.response) {
        ...
        setServerError(err.response.data?.detail || err.response.data?.non_field_errors?.[0] || 'Invalid email or password.')
      }
    } finally {
      setLoading(false)
    }
  ...
  return (
    <>
    <Helmet>
      <title>Sign In — BounceTrap</title>
      <meta name="description" content="Sign in to your BounceTrap account to verify emails and manage your list cleaning jobs." />
        ...
        <div className="bg-white/[0.04] border border-white/8 rounded-2xl p-8 shadow-2xl backdrop-blur-sm">
          ...
          <div className="relative mb-6">
            ...
            <div className="relative flex justify-center">
              <span className="px-3 text-xs text-slate-500 bg-[#0e0e1c]">or continue with email</span>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} noValidate className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-2.5 bg-brand-600 hover:bg-brand-500 disabled:opacity-60 disabled:cursor-not-allowed text-white font-semibold rounded-xl transition-all shadow-lg shadow-brand-900/30 mt-2"
            >
              {loading ? (
